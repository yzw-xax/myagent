...
# loaded from ~/.myclaw/.env). Reading them bypasses the env_config boundary.
...
class Read(BaseTool):
    ...
    def _is_credential_path(self, absolute_path: str) -> bool:
        ...
        Beyond the literal ~/.myclaw/.env file, this also blocks two real bypass
        surfaces reported in issue #2913:
          1. /proc/<pid|self|thread-self>/environ — a second view of the
             process environment that leaks secrets loaded from ~/.myclaw/.env.
          2. Symlinks resolving to ~/.myclaw/.env; the previous exact abspath
        ...
        intentionally narrowed to ~/.myclaw/.env.
        """
        ...
        env_real = os.path.realpath(expand_path("~/.myclaw/.env")).replace(os.sep, "/")
