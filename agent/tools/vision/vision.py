...
Provider resolution:
  - tools.vision.model (if set) means "prefer this model first; fall back to
    ...
    native provider. That provider is tried first, then the standard auto
...
# Model prefixes that natively belong to OpenAI (raw HTTP provider).
...
class Vision(BaseTool):
    ...
    def execute(self, args: Dict[str, Any]) -> ToolResult:
        ...
        if not providers:
            return ToolResult.fail(
                ...
                "Options:\n"
                "  1. Switch to a multimodal model (e.g. claude-sonnet-5, qwen3.7-plus, gemini-2.0-flash)\n"
    ...
    def _infer_provider_from_model(model_name: str) -> Optional[str]:
        ...
        # Sort by prefix length desc so longer prefixes match first
    ...
    def _route_by_provider_id(self, provider_id: str, user_model: str) -> Optional[List[VisionProvider]]:
        ...
        # OpenAI uses raw HTTP provider, not the discoverable bot path.
    ...
    def _route_by_model_name(self, user_model: str) -> Optional[List[VisionProvider]]:
        ...
        if lower.startswith(_OPENAI_MODEL_PREFIXES):
            ...
            logger.warning(f"[Vision] tools.vision.model='{user_model}' looks like an OpenAI "
                           f"model but OPENAI_API_KEY is not configured.")
            return None  # fall through to auto

        # Discoverable native providers (DashScope, Claude, Gemini, etc.)
