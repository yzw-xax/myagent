...
# mirrors any secrets loaded into the process environment from ~/.myclaw/.env.
...
class Grep(BaseTool):
    ...
    def __init__(self, config: dict = None):
        ...
        # Resolved once (assumes ~/.myclaw is stable for this session), not per-check.
        self._myclaw_dir = os.path.realpath(expand_path("~/.myclaw")).replace(os.sep, "/")
    ...
    def _is_credential_path(self, absolute_path: str) -> bool:
        """Block ~/.myclaw (like ls.py) and /proc/*/environ (like read.py #2913)."""
        candidates = set()
        try:
        ...
        for candidate in candidates:
            if _PROC_ENVIRON_RE.match(candidate):
                return True
        return any(c == self._myclaw_dir or c.startswith(self._myclaw_dir + "/") for c in candidates)
