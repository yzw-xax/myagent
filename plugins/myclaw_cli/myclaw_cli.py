"""
MyClawCli plugin - Intercept myclaw/slash commands in chat messages.

Matches messages like:
  myclaw skill list
  myclaw install-browser
  /skill list
  /context clear
  /status
  /install-browser
...
# Known top-level subcommands that myclaw supports.
...
@plugins.register(
    name="myclaw_cli",
    desc="Handle myclaw/slash commands in chat messages",
    version="0.1.0",
    author="MyClaw",
    desire_priority=1000,
)
class MyClawCliPlugin(Plugin):
    ...
    def _parse_command(self, content: str):
        """
        Parse myclaw command from message text.

        Supported formats:
          myclaw <command> [args...]   e.g. "myclaw skill list"
        ...
        if content.startswith("myclaw "):
            ...
            if cmd not in KNOWN_COMMANDS:
                # 'myclaw xxx' that isn't a command — don't intercept (could be
                # natural language like "myclaw xxx 怎么样").
                return None
            args = parts[1] if len(parts) > 1 else ""
            return cmd, args
    ...
    def _resolve(self, token: str, user_args: str):
        ...
        is trivially unit-testable. Note the `myclaw ` form never reaches stages
    ...
    def execute(self, query: str, session_id: str = "") -> str:
        """Execute a myclaw/slash command string without a channel context.
    ...
    def _dispatch(self, cmd: str, args: str, e_context: EventContext, session_id: str = "") -> str:
        if cmd in CLI_ONLY_COMMANDS:
            return _t(
                f"⚠️ `myclaw {cmd}` 只能在命令行终端中执行。\n请在终端运行: myclaw {cmd}",
                f"⚠️ `myclaw {cmd}` can only run in a terminal.\nRun it in your shell: myclaw {cmd}",
            )
    ...
    def _cmd_help(self, args: str, e_context, **_) -> str:
        if _t("zh", "en") == "en":
            lines = [
                ...
                "",
                "💡 You can also use myclaw <command> instead of /<command>",
            ]
        else:
            lines = [
                "📋 MyClaw 命令列表",
                ...
                "",
                "💡 也可以用 myclaw <command> 代替 /<command>",
            ]
        return "\n".join(lines)

    def _cmd_version(self, args: str, e_context, **_) -> str:
        return f"MyClaw v{__version__}"
    ...
    def _cmd_tasks(self, args: str, e_context, session_id: str = "", **_) -> str:
        ...
        if task_store is None:
            ...
            workspace = expand_path(conf().get("agent_workspace", "~/myclaw"))
    ...
    def _cmd_status(self, args: str, e_context: EventContext, session_id: str = "", **_) -> str:
        ...
        cfg = conf()
        lines = [_t("📊 MyClaw 运行状态", "📊 MyClaw Status"), ""]
    ...
    def _config_set(self, key: str, value_str: str) -> str:
        ...
        # Write to the data dir (~/.myclaw in desktop, project root otherwise), not
    ...
    # ------------------------------------------------------------------
    # install-browser (shared logic with myclaw install-browser CLI)
    # ------------------------------------------------------------------
    ...
    def _cmd_install_browser(self, args: str, e_context, **_) -> str:
        from plugins.myclaw_cli.cli.commands.install import run_install_browser

        if args.strip():
            return _t(
                "用法: /install-browser\n\n"
                "无需参数，等同于终端执行 `myclaw install-browser`。\n"
                "安装过程可能持续数分钟；进度会以多条消息推送，pip 详细输出见服务日志。",
                "Usage: /install-browser\n\n"
                "No arguments needed; equivalent to running `myclaw install-browser` in a terminal.\n"
        ...
        if code != 0:
            return _t(
                ...
                "也可在终端执行 `myclaw install-browser`。",
                "❌ Installation did not finish successfully. Check the messages above or the server log; "
                "you can also run `myclaw install-browser` in a terminal.",
            )
        return _t(
            "✅ 安装流程已结束。请重启 MyClaw 后使用 browser 工具。",
            "✅ Installation finished. Restart MyClaw to use the browser tool.",
        )
    ...
    def _skill_list_local(self) -> str:
        from plugins.myclaw_cli.cli.utils import load_skills_config, get_skills_dir, get_builtin_skills_dir
    ...
    def _skill_list_remote(self, page: int = 1) -> str:
        import requests
        from plugins.myclaw_cli.cli.utils import SKILL_HUB_API, load_skills_config
        ...
        lines.append(_t("🌐 https://skills.myclaw.ai  在线浏览全部技能", "🌐 https://skills.myclaw.ai  Browse all skills online"))
        return "\n".join(lines)
    ...
    def _skill_install(self, name: str, e_context: EventContext) -> str:
        ...
        from plugins.myclaw_cli.cli.commands.skill import install_skill
    ...
    def _format_install_result(result) -> str:
        ...
        from plugins.myclaw_cli.cli.commands.skill import _read_skill_description
        from plugins.myclaw_cli.cli.utils import get_skills_dir, load_skills_config
        skills_dir = get_skills_dir()
        config = load_skills_config()
    ...
    def _skill_uninstall(self, name: str) -> str:
        ...
        import shutil
        import json
        from plugins.myclaw_cli.cli.utils import get_skills_dir
    ...
    def _skill_info(self, name: str) -> str:
        ...
        from plugins.myclaw_cli.cli.utils import get_skills_dir, get_builtin_skills_dir
    ...
    def _resolve_active_embedding():
        ...
        if not provider_key:
            # Legacy auto path: openai, default text-embedding-3-small (1536).
    ...
    def _create_standalone_flush_manager():
        ...
        workspace = Path(expand_path(conf().get("agent_workspace", "~/myclaw")))
    ...
    def _knowledge_toggle(self, enabled: bool) -> str:
        ...
        # Data dir (~/.myclaw in desktop), not __file__-relative — see _config_set.
        from config import get_data_root
        config_path = os.path.join(get_data_root(), "config.json")
        try:
            with open(config_path, "r", encoding="utf-8") as f:
    ...
    def _knowledge_stats(self) -> str:
        ...
        knowledge_dir = os.path.join(
            expand_path(conf().get("agent_workspace", "~/myclaw")),
            "knowledge"
        )
    ...
    def get_help_text(self, **kwargs):
        return _t("在对话中使用 /help 或 myclaw help 查看可用命令", "Use /help or myclaw help in chat to see available commands")
