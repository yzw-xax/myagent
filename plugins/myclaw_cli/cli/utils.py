"""Shared utilities for myclaw CLI (copied under plugins/myclaw_cli/cli)."""
...
def get_project_root() -> str:
    """Get the MyClaw project root directory.

    We live under plugins/myclaw_cli/cli/ — walk up 3 levels to reach root.
    """
...
def get_workspace_dir() -> str:
    """Get the agent workspace directory from config, defaulting to ~/myclaw."""
...
def get_cli_language() -> str:
    ...
    Reads the `myclaw_lang` field from config.json (defaults to "auto") and runs
    ...
    try:
        ...
        configured = load_config_json().get("myclaw_lang", "auto")
...
SKILL_HUB_API = "https://skills.myclaw.ai/api"
