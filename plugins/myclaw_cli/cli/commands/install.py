"""myclaw install-browser - Install Playwright + Chromium for the browser tool.

Copied from root cli/commands/install.py into plugins/myclaw_cli/cli/commands/install.py
so the chat plugin no longer depends on the removed root cli package.
"""
...
def run_install_browser(
    ...
    Reused by chat ``/install-browser``.
    """
    ...
    stream("")
    stream("Browser tool ready! Restart MyClaw to enable it.", "green")
    _phase(on_phase, _t(
        "🎉 全部步骤结束。请重启 MyClaw 后使用 browser 工具。",
        "🎉 All steps finished. Restart MyClaw to use the browser tool.",
    ))
    return 0
