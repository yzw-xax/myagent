"""myclaw skill - Skill management commands."""
...
from plugins.myclaw_cli.cli.utils import (
    get_project_root,
    get_skills_dir,
...
def _clone_repo(git_url: str):
    ...
    """
    tmp_dir = tempfile.mkdtemp(prefix="myclaw-skill-")
...
def _register_installed_skill(name: str, source: str = "cowhub", display_name: str = ""):
    ...
    source values: builtin, myclaw, github, clawhub, local, url
    """
    skills_dir = get_skills_dir()
...
def _print_install_success(name: str, source: str):
    ...
    from plugins.myclaw_cli.cli.utils import get_cli_language

    # Import `common` only after get_cli_language() runs ensure_sys_path(),
    # so it works when `myclaw` is invoked from outside the project directory.
    get_cli_language()  # resolve myclaw_lang so i18n.t reflects config
...
def _list_remote(page: int = 1):
    ...
    nav_parts = []
    if page > 1:
        nav_parts.append(f"myclaw skill list --remote --page {page - 1}")
    if page < total_pages:
        nav_parts.append(f"myclaw skill list --remote --page {page + 1}")
    if nav_parts:
        click.echo(f"  Navigate: {' | '.join(nav_parts)}")
    click.echo(f"  Install:  myclaw skill install <name>")
    click.echo(f"  Browse:   https://skills.myclaw.ai\n")
...
def search(query):
    ...
    click.echo(f"\n  Install with: myclaw skill install <name>\n")
...
# ------------------------------------------------------------------
# myclaw skill install (CLI thin wrapper)
# ------------------------------------------------------------------
@skill.command()
@click.argument("name")
def install(name):
    ...
    Examples:
      ...
      myclaw skill install pptx                          (from Skill Hub)

      myclaw skill install larksuite/cli                 (GitHub shorthand, all skills)

      myclaw skill install larksuite/cli#skills/lark-im  (single skill by subpath)

      myclaw skill install https://github.com/owner/repo

      myclaw skill install https://gitlab.com/org/repo

      myclaw skill install git@github.com:owner/repo.git

      myclaw skill install ./my-local-skills             (local directory)

      myclaw skill install https://example.com/path/to/SKILL.md
    """
