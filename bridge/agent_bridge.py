"""
Agent Bridge - Integrates Agent system with existing MYCLAW bridge
"""
...
class AgentLLMModel(LLMModel):
    """
    LLM Model adapter that uses MYCLAW's existing bot infrastructure
    """
    ...
    def call(self, request: LLMRequest):
        """
        Call the model using MYCLAW's bot infrastructure
        """
        try:
    ...
    def call_stream(self, request: LLMRequest):
        """
        Call the model with streaming using MYCLAW's bot infrastructure
        """
        try:
            if hasattr(self.bot, 'call_with_tools'):
...
class AgentBridge:
    """
    Bridge class that integrates super Agent with MYCLAW
    ...
    def create_agent(self, system_prompt: str, tools: List = None, **kwargs) -> Agent:
        """
        Create the super agent with MYCLAW integration
        ...
        Returns:
            Agent instance
        """
        # Create LLM model that uses MYCLAW's bot infrastructure
    ...
    def agent_reply(self, query: str, context: Context = None, 
        ...
        Args:
            query: User query
            context: MYCLAW context (optional, contains session_id for user isolation)
    ...
    def _migrate_config_to_env(self, workspace_root: str):
        ...
        env_file = expand_path("~/.myclaw/.env")
    ...
    def refresh_all_skills(self) -> int:
        ...
        workspace_root = expand_path(conf().get("agent_workspace", "~/myclaw"))
