...
def _print_qr_to_terminal(qr_url: str):
    ...
    走 logger 而非 print 是为了避免 nohup/myclaw 后台启动场景下 stdout 块缓冲导致
...
class FeiShuChanel(ChatChannel):
    ...
    # 覆盖父类默认值，飞书原生支持发送图片，置为空列表以启用图片回复。
    ...
    def _get_scheduler_task_store(self):
        ...
        workspace_root = utils.expand_path(conf().get("agent_workspace", "~/myclaw"))
