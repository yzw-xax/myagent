/* =====================================================================
   MyClaw Console - Main Application Script
   ===================================================================== */
...
const I18N = {
    zh: {
        ...
        config_tab_basic: '基础配置', config_tab_models: '模型管理',
    ...
    en: {
        ...
        menu_logs: 'Logs',
        models_title: 'Models',
        models_desc: 'Manage chat, image, embedding and search capabilities in one place',
...
// (myclaw_lang) -> browser language -> 'zh'. Shares __cowResolveLang__ defined in
...
function applyI18n() {
    ...
    if (docsLink) docsLink.href = currentLang === 'zh' ? 'https://docs.myclaw.ai/zh' : 'https://docs.myclaw.ai';
}
...
// the backend `myclaw_lang` config so logs / agent replies / CLI follow suit.
function setLanguage(lang) {
    ...
    currentLang = next;
    localStorage.setItem('myclaw_lang', currentLang);
    applyI18n();
    _applyInputTooltips();
...
// Persist the language to the backend `myclaw_lang` config (best-effort; the UI
...
function syncLanguageToBackend(lang, callback) {
    try {
        fetch('/config', {
            ...
            body: JSON.stringify({ updates: { myclaw_lang: lang } })
        })
...
// Theme
// =====================================================================
let currentTheme = localStorage.getItem('myclaw_theme') || 'dark';
...
function toggleTheme() {
    ...
    localStorage.setItem('myclaw_theme', currentTheme);
    applyTheme();
}
...
function switchConfigTab(tab) {
    ...
    const tabEl = document.getElementById('config-tab-' + tab);
    if (tabEl) tabEl.classList.add('active');

    document.querySelectorAll('.config-panel').forEach(el => el.classList.add('hidden'));
    const panelEl = document.getElementById('config-panel-' + tab);
    if (panelEl) panelEl.classList.remove('hidden');

    currentConfigTab = tab;

    if (tab === 'models') {
        if (typeof loadModelsView === 'function') {
            loadModelsView();
        }
    }
}
...
function navigateTo(viewId) {
    if (viewId === 'models') {
        viewId = 'config';
        setTimeout(() => switchConfigTab('models'), 0);
    }
    if (viewId === 'channels') {
        viewId = 'config';
    }
    ...
    if (viewId === 'config') {
        setTimeout(() => switchConfigTab(currentConfigTab || 'basic'), 0);
    }
...
function _openImageLightbox(src) {
    let overlay = document.getElementById('myclaw-lightbox');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'myclaw-lightbox';
    ...
    overlay.querySelector('#myclaw-lightbox-img').src = src;
...
let appConfig = { use_agent: false, title: 'MyClaw', subtitle: '', providers: {}, api_bases: {} };

const SESSION_ID_KEY = 'myclaw_session_id';
...
function startSSE(requestId, loadingEl, timestamp, titleInfo, replayItems) {
    ...
    function ensureBotEl() {
        ...
        botEl.innerHTML = `
            <svg viewBox="0 0 64 64" class="w-8 h-8 rounded-lg flex-shrink-0" xmlns="http://www.w3.org/2000/svg"><rect width="64" height="64" rx="16" fill="#7C3AED"/><path d="M16 44 C20 34, 24 26, 36 18" stroke="white" stroke-width="3" stroke-linecap="round" fill="none"/><path d="M14 48 C18 38, 22 30, 34 22" stroke="white" stroke-width="3" stroke-linecap="round" fill="none"/><path d="M12 52 C16 42, 20 34, 32 26" stroke="white" stroke-width="3" stroke-linecap="round" fill="none"/><circle cx="38" cy="16" r="2.5" fill="white" opacity="0.8"/></svg>
    ...
    // Holds the live EventSource so terminal events (done/error)
    ...
    function processSSEItem(item) {
            if (item.type === 'reasoning') {
                ...
                // Coarse progress (e.g. myclaw install-browser); must not close SSE (unlike "done")
                ensureBotEl();
                ...
                // for a short tail to deliver async attachments.
                // It will close the stream on its own via onerror
                // once the tail expires.
                done = true;
                clearOwnerRequest();
                resetSendBtnSendMode();
...
function newChat(optimistic = true) {
    ...
    ws.innerHTML = `
        <svg viewBox="0 0 64 64" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20" xmlns="http://www.w3.org/2000/svg"><rect width="64" height="64" rx="16" fill="#7C3AED"/><path d="M16 44 C20 34, 24 26, 36 18" stroke="white" stroke-width="3" stroke-linecap="round" fill="none"/><path d="M14 48 C18 38, 22 30, 34 22" stroke="white" stroke-width="3" stroke-linecap="round" fill="none"/><path d="M12 52 C16 42, 20 34, 32 26" stroke="white" stroke-width="3" stroke-linecap="round" fill="none"/><circle cx="38" cy="16" r="2.5" fill="white" opacity="0.8"/></svg>
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || 'MyClaw'}</h1>
...
const SESSION_PANEL_KEY = 'myclaw_session_panel_open';
...
function initDropdown(el, options, selectedValue, onChange, opts) {
    ...
    // "pick or empty" capabilities (e.g. embedding) where we want the
...
function loadModelsView(opts) {
    ...
    // The Models pane is now inside the Config view; capture its scroll position
...
function buildCapabilityProviderOptions(def, cap) {
    ...
    // unconfigured row opens the vendor modal in-place. Engines that
    // aren't tracked by PROVIDER_MODELS are treated
...
function rebuildCapabilityModelDropdown(def, providerId, selectedModel, scope) {
    ...
    // embedding where any vendor model is fair game.
    //
...
function buildFeishuPanel(ch, isActive) {
    ...
    const isActivePanel = isActive;
    return `
        <div id="feishu-panel" data-default-mode="manual" ${activeAttr}>
...
function resolveKnowledgePath(currentFilePath, relativeHref) {
    // currentFilePath: e.g. "concepts/example-protocol.md"
