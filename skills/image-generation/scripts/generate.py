...
OpenAI → Gemini → Qwen; missing API keys are skipped, and the provider
that natively owns the requested model is promoted to the front of the
queue):
...
_DEFAULT_PROVIDER_ORDER = ["OpenAI", "Gemini", "Qwen"]
...
def _build_providers(model: str, provider_id: str = "") -> list[tuple[str, ImageProvider]]:
    ...
    Behaviour:
      1. All providers with a configured API key are added in the global
         priority order: OpenAI → Gemini → Qwen.
...
def main():
    ...
    if not providers:
        ...
        print(json.dumps({
            "error": (
                ...
                "DASHSCOPE_API_KEY "
                "via the env_config tool, then try again."
            )
        }, ensure_ascii=False))
        sys.exit(1)
    ...
    print(json.dumps({
        "error": f"All providers failed — {hint}. "
                 ...
                 "(OPENAI_API_KEY, GEMINI_API_KEY, or DASHSCOPE_API_KEY) "
                 "via env_config."
    }, ensure_ascii=False))
    sys.exit(1)
